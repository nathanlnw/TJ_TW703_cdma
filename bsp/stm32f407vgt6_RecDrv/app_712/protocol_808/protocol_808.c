	  ...
	  if(strncmp(ISP_buffer+32+13,"70420TW703",10)==0) //判断厂商和型号
		 {
			 ...
			 if(HardWareVerion==HardVersion)											 	
			 	 ...
			 	 rt_kprintf("\r\n 硬件版本不匹配!\r\n");  
			 //firmware
			 if(strncmp(ISP_buffer+32+42,"NXGGHYPT",8)==0) 
			 {
				ISP_judge_resualt++;// step 3
				rt_kprintf("\r\n  固件版本:NXGGHYPT\r\n");	 
			 }
			 // operater
			 if(strncmp(ISP_buffer+32+62,"NXCDMA",6)==0) 
			 {
...
void TCP_RX_Process( u8  LinkNum)  //  ---- 808  标准协议 
{
  ...
  //  4.   Check  Fcs 
	  ...
	  if( UDP_HEX_Rx[UDP_DecodeHex_Len-2]!=FCS_RX_UDP ) //判断校验结果	    
		  {
			  rt_kprintf("\r\n  Protocolinfolen=%d   UDP_hexRx_len=%d   UDP_DecodeRx_len=%d",infolen,UDP_hexRx_len,UDP_DecodeHex_Len);    
                                   ...
                                   if(strncmp(BD_ISP.ContentData+32+13,"70420TW703",10)==0) //判断厂商和型号
                                   	{
										...
										//firmware
										if(strncmp(BD_ISP.ContentData+32+42,"NXGGHYPT",8)==0) 
										{
										   ISP_judge_resualt++;// step 3
										   rt_kprintf("\r\n  固件版本:NXGGHYPT\r\n");   
										}
										// operater
										if(strncmp(BD_ISP.ContentData+32+62,"NXCDMA",6)==0)  
										{
										   ISP_judge_resualt++;// step 4
										   rt_kprintf("\r\n  固件版本:NXCDMA\r\n");      
										}
...
void Time2BCD(u8 *dest)
 {
    ...
    if(UDP_dataPacket_flag==0x02)
   {
        // rt_kprintf("\r\n set %d-%d-%d %d:%d:%d\r\n",Temp_Gps_Gprs.Date[0],Temp_Gps_Gprs.Date[1],Temp_Gps_Gprs.Date[2],\
		 	       Temp_Gps_Gprs.Time[0],Temp_Gps_Gprs.Time[1],Temp_Gps_Gprs.Time[2]);
		 
		   six[0]=((Temp_Gps_Gprs.Date[0]/10)<<4)+(Temp_Gps_Gprs.Date[0]%10);  
		   six[1]=((Temp_Gps_Gprs.Date[1]/10)<<4)+(Temp_Gps_Gprs.Date[1]%10);//Temp_Gps_Gprs.Date[1];
		   six[2]=((Temp_Gps_Gprs.Date[2]/10)<<4)+(Temp_Gps_Gprs.Date[2]%10);//Temp_Gps_Gprs.Date[2]; 
		   
		   six[3]=((Temp_Gps_Gprs.Time[0]/10)<<4)+(Temp_Gps_Gprs.Time[0]%10);//Temp_Gps_Gprs.Time[0]; 
		   six[4]=((Temp_Gps_Gprs.Time[1]/10)<<4)+(Temp_Gps_Gprs.Time[1]%10);//Temp_Gps_Gprs.Time[1]; 
		   six[5]=((Temp_Gps_Gprs.Time[2]/10)<<4)+(Temp_Gps_Gprs.Time[2]%10);//Temp_Gps_Gprs.Time[2];   
	   
       //rt_kprintf("\r\n set over %x-%x-%x %x:%x:%x\r\n",six[0],six[1],six[2],six[3],six[4],six[5]);   
   }
   else
   {
		six[0]=((time_now.year/10)<<4)+(time_now.year%10);  	   	
		six[1]=((time_now.month/10)<<4)+(time_now.month%10); 
		six[2]=((time_now.day/10)<<4)+(time_now.day%10);
		six[3]=((time_now.hour/10)<<4)+(time_now.hour%10);
		six[4]=((time_now.min/10)<<4)+(time_now.min%10);
		six[5]=((time_now.sec/10)<<4)+(time_now.sec%10);    
	}
